...
simLong <- function(r, J=6, n=6000, t.end=2, gbound=c(0.01,0.99), ybound=c(0.0001, 0.9999), n.folds=10, estimator=c("lmtp","ltmle","tmle"), treatment.rule = c("static","dynamic","stochastic","all"), use.SL=TRUE){
  ...
  if(estimator=="tmle"){
    ...
    tmle_dat <- data.frame(tmle_dat[tmle_dat$t>0,]) # discard baseline (V's are constant and baseline L's are in the lags)
  ...
  results_CI_ltmle <- list() 
  results_bias_ltmle <- list() 
  results_CP_ltmle <- list()
  results_CIW_ltmle <- list()
  ...
  if(estimator=="ltmle"){
    ...
    Chat_ltmle <- ltmle_results[[treatment.rule]]$cum.g[,c((J+1)*rep(1:t.end))] 
